...
# TODO sometimes error 0x882ccad9: RedeemZeroLots in redeem function on contract
class RedeemRandomAmount(ActionBundle):
    ...
    def action(self):
        # action logic
        lot_amount = 1 # random.randint(1, self.balances[self.token_fasset] // self.lot_size)
...
class RedeemDefaultRandomRedemption(ActionBundle):
    ...
    def expected_state(self):
        ...
        redemption_fee = AssetManager(self.token_underlying).redemption_fee_bips()
        new_balances = self.balances.copy()
        new_balances[self.token_underlying] += self.lot_size * lot_amount * (1 - redemption_fee / 100)
        new_balances.subtract_fees(self.ca.fee_tracker)
        # redemption status
        new_redemption_status = self.redemption_status.copy()
